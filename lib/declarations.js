...
, { name: 'getQueue'
  ...
  , args: ['pid', 'range']
  , success: data => ({
      count: parseInt (data.heos.message.count, 10)
    , items: data.payload
    }) 
  }
...
, { name: 'addContainerToQueue'
  ...
  , args: ['sid', 'cid', 'aid' /* 1:now, 2:next, 3:last, 4:replace */, 'pid']
  , success: getMessage
  }

, { name: 'addTrackToQueue'
  , command: 'browse/add_to_queue'
  , args: ['sid', 'cid', 'mid', 'aid' /* 1:now, 2:next, 3:last, 4:replace */, 'pid']
  , success: getMessage
  }
...
, { name: 'getHeosPlaylists'
  , func:  async function () {
      ...
      return this.browseSource (playlists_source.sid)
    }
  }
...
, { name: 'getHeosHistory'
  , func: async function (type) {
      ...
      const typed = response.data.find (matchingProps ({name: type}, false))
      return this.browseSourceContainers (history_source.sid, typed.cid)
    }
  }
